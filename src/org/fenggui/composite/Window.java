...
public class Window extends Container implements IWindow
{
  ...
  private ArrayList<IWindowResizedListener> windowResizedHook      = new ArrayList<IWindowResizedListener>();
  ...
  protected void buildMinimizeButton()
  {
    ...
    minimizeButton.addButtonPressedListener(new IButtonPressedListener()
    {
      ...
      {
        throw new UnsupportedOperationException("Minimize Window: Not implemented yet");
      }
    });
  ...
  class WindowMoveDnDListenerImpl implements IDragAndDropListener
  {
    ...
    public void drag(int x, int y, Set<Key> modifiers)
    {
      ...
      if (x < 0 || y < 0 || x > getParent().getSize().getWidth() || y > getParent().getSize().getHeight())
        return;
  ...
  private void fireResizedEvent(int oldWidth, int oldHeight)
  {
    ...
    {
      WindowResizedEvent event = new WindowResizedEvent(this, oldWidth, oldHeight);
      for (IWindowResizedListener listener : windowResizedHook)
      {
  ...
  /**
   * Add a {@link IWindowResizedListener} to the widget. The listener can be added only once.
   * @param l
   */
  ...
  public void process(InputOutputStream stream) throws IOException, IXMLStreamableException
  {
    ...
    setMinSize(stream.processAttribute("minWidth", getMinWidth(), 50), stream.processAttribute("minHeight",
      getMinHeight(), 50));
