 ...
 * all grabbing child widgets. Widgets need to have the RowLayoutData set or it will use the default 
 * (RowLayoutData.DEFAULT). The layout is able to grab available space using weights. It is also able
 * to resize widgets to there full available or to align the widgets.
 * 
 * <p>
  ...
  private void placeWidgets(Container container, List<IWidget> widgets)
  {
    ...
    if (horizontal)
    {
      for (IWidget widget : widgets)
      {
        ...
        widget.setY(widgetData.getAlign().alignY(container.getAppearance().getContentHeight(),
    ...
    else
    {
      for (int i = (widgets.size() - 1); i >= 0; i--)
      {
        ...
        widget.setX(widgetData.getAlign().alignX(container.getAppearance().getContentWidth(),
  ...
  private void computeIndividualSpace(Container container, List<IWidget> widgets, int spaceLeft)
  {
    ...
    for (IWidget widget : widgets)
    {
      ...
      if (widgetData.isGrab())
      {
        if (horizontal)
        {
          this.setValidSize(widget, (int) ((sizeUnified * widgetData.getWeight())), widget.getMinSize().getHeight());
        }
        else
        {
          this.setValidSize(widget, widget.getMinSize().getWidth(), (int) ((sizeUnified * widgetData.getWeight())));
        }
      }
      else
      {
