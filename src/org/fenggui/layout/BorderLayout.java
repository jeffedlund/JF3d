 ...
 * $Id: BorderLayout.java 606 2009-03-13 14:56:05Z marcmenghin $
 */
...
public class BorderLayout extends LayoutManager
{
  ...
  public void doLayout(Container container, List<IWidget> content)
  {
    ...
    for (IWidget w : content)
    {
      ...
      if (bld.getValue() == BorderLayoutData.NORTH_VALUE)
      {
        ...
        setValidSize(w, container.getAppearance().getContentWidth() - widthWest - widthEast, getValidMinHeight(w));
      }
      else if (bld.getValue() == BorderLayoutData.SOUTH_VALUE)
      {
        w.setX(widthWest);
      ...
      else if (bld.getValue() == BorderLayoutData.EAST_VALUE)
      {
        ...
        w.setY(0);
        setValidSize(w, getValidMinWidth(w), container.getAppearance().getContentHeight());
      }
      else if (bld.getValue() == BorderLayoutData.WEST_VALUE)
      {
        w.setX(0);
      ...
      { // CENTER
        w.setX(widthWest);
        w.setY(heightSouth);
        setValidSize(w, container.getAppearance().getContentWidth() - widthEast - widthWest, container.getAppearance()
            .getContentHeight()
  ...
  public int getMinHeight(List<IWidget> content)
  {
    ...
    for (IWidget c : content)
    {
      ...
      if (bld.getValue() == BorderLayoutData.CENTER_VALUE || bld.getValue() == BorderLayoutData.NORTH_VALUE
  ...
  public int getMinWidth(List<IWidget> content)
  {
    ...
    for (IWidget c : content)
    {
      ...
      if (bld.getValue() == BorderLayoutData.CENTER_VALUE || bld.getValue() == BorderLayoutData.NORTH_VALUE
          || bld.getValue() == BorderLayoutData.SOUTH_VALUE)
      {
        if (middleColumn < getValidMinWidth(c))
          middleColumn = getValidMinWidth(c);
      }
      else if (bld.getValue() == BorderLayoutData.EAST_VALUE)
      {
