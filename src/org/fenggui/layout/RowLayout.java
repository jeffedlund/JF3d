 ...
 * 
 * $Id: RowLayout.java 606 2009-03-13 14:56:05Z marcmenghin $
 */
...
public class RowLayout extends LayoutManager
{
  ...
  public void doLayout(Container container, List<IWidget> content)
  {
    ...
    int reqSpace = horizontal ? getSumOfAllWidths(content) : getSumOfAllHeights(content);

    freeSpacePerComp = horizontal ? container.getAppearance().getContentWidth() : container.getAppearance()
        .getContentHeight();
    ...
    for (int i = 0; i < content.size(); i++)
    {
      ...
      if (horizontal)
      {
        ...
        setValidSize(w, (int) freeSpacePerComp + getValidMinWidth(w), container.getAppearance().getContentHeight());
      ...
      else
      {
        ...
        setValidSize(w, container.getAppearance().getContentWidth(), (int) freeSpacePerComp + getValidMinHeight(w));
