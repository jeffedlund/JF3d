...
public class FormLayout extends LayoutManager
{
  ...
  private void sort(IWidget w, String offset)
  {
    try
    {
      ...
      {
        if (debug)
          System.out.println("oh, " + w + " is already in the order list");
        return;
      }
    ...
    {
      System.err.println("There seems to be a cyclic dependency " + "for widget " + w + "\n" + soe.getMessage());
      System.exit(-1);
    }
  }
  ...
  public void doLayout(Container container, List<IWidget> content)
  {
    ...
    for (IWidget w : content)
    {
      ...
      {
        if (debug)
          System.out.println("fd.bottom: " + w.getClass().getSimpleName() + " " + fd.bottom.isStatic());
    ...
    for (IWidget w : order)
    {
      ...
      if (fd.left != null && !fd.left.isStatic())
      {
        int rightSideOfAttachedWidget = fd.left.getAttachedWidget().getX() + fd.left.getAttachedWidget().getWidth()
            + fd.left.getOffset();
      ...
      if (fd.bottom != null && !fd.bottom.isStatic())
      {
        int topSideOfAttachedWidget = fd.bottom.getAttachedWidget().getY() + fd.bottom.getAttachedWidget().getHeight()
            + fd.bottom.getOffset();
