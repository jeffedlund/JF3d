...
public class XMLOutputStream extends OutputOnlyStream
{
  ...
  public XMLOutputStream(OutputStream out, String rootElementName, GlobalContextHandler contextHandler)
  {
  ...
  public XMLOutputStream(OutputStream out, String rootElementName, GlobalContextHandler contextHandler, boolean saveDefaultValues, int numSavedDecimals)
  {
  ...
  private <T extends IXMLStreamable> Element handleChild(T child, String elementName) throws IOException,
      IXMLStreamableException
  {
    ...
    else
    {
      ...
      if (wasWritten)
      {
        String setName = activeElement.getAttributeValue(XMLInputStream.ATTRIBUTE_NAME);
  ...
  @Override
  public <T extends IXMLStreamable> T processChild(String name, T value, Class<T> objectClass) throws IOException,
      IXMLStreamableException
  {
    if (value == null)
      return null;
  ...
  public <T extends IXMLStreamable> void processChildren(String childName, Map<String, T> children, Class<T> childClass)
      throws IOException, IXMLStreamableException
  {
    for (Entry<String, T> entry : children.entrySet())
    {
      Element element = handleChild((IXMLStreamable) entry.getValue(), childName);
      element.setAttribute(InputOutputStream.KEY_MAPKEY, entry.getKey());
    }
  }

  /* (non-Javadoc)
   * @see org.fenggui.theme.xml.InputOutputStream#processChildren(java.lang.String, java.util.Map, java.lang.Class, java.lang.Enum)
   */
  @Override
  public <T extends Enum<T>> void processEnumChildren(String childName, Map<String, T> children, Class<T> enumClass) throws IOException, IXMLStreamableException
  {
    for (Entry<String, T> entry : children.entrySet())
    {
      ...
      //element.setAttribute(InputOutputStream.KEY_MAPKEY, entry.getValue().name());
      element.setAttribute(InputOutputStream.KEY_MAPKEY, entry.getValue().toString());
    }
  }
      ...
      throws IOException, IXMLStreamableException
  {
    for (Entry<String, T> entry : children.entrySet())
    {
      String name = typeRegister.getName((Class<? extends IXMLStreamable>) entry.getValue().getClass());

      if (name != null)
      {
        Element element = handleChild((IXMLStreamable) entry, name);
        element.setAttribute(InputOutputStream.KEY_MAPKEY, entry.getKey());
      }
    }
