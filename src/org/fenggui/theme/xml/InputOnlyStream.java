...
public abstract class InputOnlyStream extends InputOutputStream
{
   ...
   */
  public <T extends IXMLStreamable> T constructObject(Class<T> clazz) throws IXMLStreamableException, IOException
  {
    try
    {
      try
      {
        Constructor<T> constr = clazz.getConstructor(InputOnlyStream.class);
        return constr.newInstance(this);
      }
      catch (NoSuchMethodException e)
      {
        addWarning("Used the default constructor for class " + clazz.getName());
